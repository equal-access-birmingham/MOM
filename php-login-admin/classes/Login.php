...
class Login
{
	...
	public function addVerificationInfo($user_id, $user_name, $user_password_old, $user_password_new, $user_password_repeat, $fname, $mname, $lname, $suffname, $gender, $dob_month, $dob_day, $dob_year, $area_code, $region_code, $last_four, $school_id, $level_id)
	{
		...
		if(empty($user_id)) {
			...
			// Checks to see if everything went into the database
			$person_info_count = $query_personal_info->rowCount();

			// Grabs the person_id for insertion into the login_relation_table
			...
			$query = "INSERT INTO `login_relation_table` VALUES ('', :person_id, :user_id);";
			...
			// Checks to see if the login_info was properly inserted into the login_relation_table
			$login_info_count = $query_login_info->rowCount();

			// Grabs the login_relation_id for insertion into the school_relation_table
			...
			$query = "INSERT INTO `school_relation_table` VALUES ('', :login_relation_id, :school_id, :level_id);";
			...
			// Checks to see if the academic info was correctly inserted into the school_relation_table
			$academic_info_count = $query_academic_info->rowCount();

			//Grabs the school_relation_id for possible deletion
			$school_relation_id = $this->db_connection->lastInsertId();
			...
			// Checks to see if the position info was correctly inserted into the school_relation_table
			$position_info_count = $query_position_info->rowCount();

			//Grabs the position_id for possible deletion
			$position_id = $this->db_connection->lastInsertId();

			if($person_info_count && $login_info_count && $academic_info_count && $position_info_count) {
				if($this->editUserPassword($user_password_old, $user_password_new, $user_password_repeat)) {
					...
					$this->deletePersonalInfo($person_id, $login_relation_id, $school_relation_id, $position_id);

					// Letting the editUserPassword function do error reporting here (i.e. I don't need more error reporting)
				}
			} else {
				// Personal info has failed to be uploaded, delete info that was uploaded
				$this->deletePersonalInfo($person_id, $login_relation_id, $school_relation_id, $position_id);
	 ...
	 * This is used to control verification
	 */
	private function deletePersonalInfo($person_id, $login_relation_id, $school_relation_id, $position_id)
	{
		if(empty($person_id)) {
		...
		} else if(!preg_match('/^[0-9]*$/', $person_id)) {
			$this->errors[] = "Person ID must be a number";
		} else if(empty($login_relation_id)) {
			...
			$query_delete_person_info->bindValue(':person_id', $person_id, PDO::PARAM_STR);
			$query_delete_person_info->execute();

			// Delete login info
			$query = "DELETE FROM `login_relation_table` WHERE `login_relation_id` = :login_relation_id;";
			$query_delete_login_info = $this->db_connection->prepare($query);
			$query_delete_login_info->bindValue(':login_relation_id', $login_relation_id, PDO::PARAM_STR);
			$query_delete_login_info->execute();
			
			// Delete academic info
			$query = "DELETE FROM `school_relation_table` WHERE `school_relation_id` = :school_relation_id;";
			$query_delete_academic_info = $this->db_connection->prepare($query);
			$query_delete_academic_info->bindValue(':school_relation_id', $school_relation_id, PDO::PARAM_STR);
	...
	private function denyLogin($user_name)
	{
		...
		if($this->verifyAccount($user_name) == false && $date_interval->format("%d")>= 3) {
			...
			$this->messages[] = MESSAGE_NEW_ACCOUNT_VERIFY_REQUEST . (3 - $date_interval->format("%d"));
		}
