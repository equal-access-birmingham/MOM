...
?>
    ...
    <title>Sign Up | Equal Access Birmingham</title>
...
while($result_program = $stmt_program->fetch())
{
	...
	if($date > $date_now && !in_array($result_program['program_name']))
	{
...
//grabbing dates as long as they are in the future of current date
$date = new DateTime("now");
$query_date = "SELECT `program_relation_id`, `program_id`, `date` FROM `program_relation_table` WHERE DATE(`date`) > :start_date ORDER BY `date`;";
...
if(isset($_GET['confirm']))
{
	...
	if(!$result_double_trouble['count'])
	{
		...
		{
			echo "            <p>Error: signup could not be completed. Please contact a member of the <a href=\"mailto:eabitteam@gmail.com\">EAB IT team</a>.</p>";
		}
	}
...
?>
    ...
    <div class="container no-image">
      <h1>Sign Up Now</h1>
      <form method="get" action="" name="entryForm" role="form">
...
//if program is not null, then start listing dates
        if(program != "")
        {
          date_select.options[date_select.options.length] = new Option("-- Date --", "");
          //prints all available dates for selected program
          for(var i = 0; i < eab_date_json.length; i++)
          {
            if(eab_date_json[i].program_id == program)
            {
              date_select.options[date_select.options.length] = new Option(prettyDate(eab_date_json[i].date), eab_date_json[i].program_relation_id);
            }
          }
        }
...
//if date is not null, attempt to list proper set of roles
        if(date != "")
        {
          role_select.options[role_select.options.length] = new Option("-- Role --", "");
...
//removes role from json_array when certain conditions are met
      ...
      // Changes a date of format YYYY-MM-DD to "month" D, YYYY (January 15, 1964)
      function prettyDate(date)
      {
        ...
        // Explode date into array of components for manipulation
        var date_array = date.split("-");

        // Remove any leading zeros
        for(var i = 0; i < date_array.length; i++)
        {
          if(date_array[i].substr(0, 1) == "0")
          {
            date_array[i] = date_array[i].substr(1);
          }
        }

        return months[date_array[1]-1] + " " + date_array[2] + ", " + date_array[0];
      }
